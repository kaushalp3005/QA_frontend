...
export default function DailyCleaningChecklistPrintPage() {
  ...
  const grid: Record<string, Record<number | string, string>> = record?.grid || {};
  const parameters = Object.keys(grid);
  const checkedByPerDay: Record<string, string> = record?.checked_by_per_day || {};
  const verifiedByPerDay: Record<string, string> = record?.verified_by_per_day || {};
  const title: string = record?.title || "Daily Cleaning checklist";
  const docNo: string = record?.document_no || "CFPLA.C4.F.54e";
    ...
    <div className="min-h-screen bg-gray-300 print:bg-white">
      ...
      >
        ...
        <div style={{ marginTop: "8px", marginBottom: "4px", fontSize: "12px", fontWeight: "bold" }}>
          ...
          <span style={{ marginLeft: "40px" }}>Area: <span style={{ fontWeight: "normal" }}>{record?.area || ""}</span></span>
        </div>
          ...
          <tbody>
            {parameters.map((param) => (
              <tr key={param}>
                ...
                {DAYS.map((d) => {
                  const val = grid[param]?.[d] ?? grid[param]?.[String(d)] ?? "";
                  return <td key={d} style={td}>{val}</td>;
                })}
              </tr>
            ))}
            <tr>
              ...
              {DAYS.map((d) => {
                const name = checkedByPerDay[d] ?? checkedByPerDay[String(d)] ?? "";
                return (
                  <td key={d} style={{ ...td, padding: "1px" }}>
                    <SignatureCell name={name} maxHeight={20} maxWidth={28} showName={false} />
                  </td>
                );
              })}
            </tr>
            <tr>
              ...
              {DAYS.map((d) => {
                const name = verifiedByPerDay[d] ?? verifiedByPerDay[String(d)] ?? "";
                return (
                  <td key={d} style={{ ...td, padding: "1px" }}>
                    <SignatureCell name={name} maxHeight={20} maxWidth={28} showName={false} />
                  </td>
