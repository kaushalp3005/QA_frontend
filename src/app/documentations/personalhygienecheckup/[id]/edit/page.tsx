...
function CompactSignSelect({ value, onChange, options }: { value: string; onChange: (v: string) => void; options: SignatureOption[] }) {
  ...
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className="input-base !py-1 !px-2 text-xs" title={value || "Select signatory"}>
      <option value="">—</option>
...
const GROUPS = [
  { name: "Injury/Infectious Diseases", tone: "danger"  as const, span: 4 },
  { name: "Personal Cleanliness",       tone: "info"    as const, span: 4 },
  { name: "Personal Belongings",        tone: "success" as const, span: 2 },
];

const groupHeaderClass = (tone: "danger" | "info" | "success") =>
  tone === "danger" ? "text-danger-600 bg-danger-50/70" : tone === "info" ? "text-blue-700 bg-blue-50/70" : "text-success-700 bg-success-50/70";

const cellTintClass = (tone: "danger" | "info" | "success") =>
  tone === "danger" ? "bg-danger-50/30" : tone === "info" ? "bg-blue-50/30" : "bg-success-50/30";

function normalizeRows(raw: any[]): HygieneRow[] {
  if (!Array.isArray(raw) || raw.length === 0) return [];
  return raw.map((r: any, i: number) => ({
    id: i + 1,
    name: r.name || "",
    selected: true,
    respiratory:   (r.respiratory    || "") as CheckValue,
    skinDisease:   (r.skin_disease   || "") as CheckValue,
    wounds:        (r.wounds         || "") as CheckValue,
    earNoseThroat: (r.ear_nose_throat|| "") as CheckValue,
    gowning:       (r.gowning        || "") as CheckValue,
    handHygiene:   (r.hand_hygiene   || "") as CheckValue,
    nails:         (r.nails          || "") as CheckValue,
    cleanShaven:   (r.clean_shaven   || "") as CheckValue,
    hairPins:      (r.hair_pins      || "") as CheckValue,
    tobacco:       (r.tobacco        || "") as CheckValue,
...
const emptyRow = (): HygieneRow => ({
  id: Date.now() + Math.random(), name: "", selected: true,
  respiratory: "", skinDisease: "", wounds: "", earNoseThroat: "",
  gowning: "", handHygiene: "", nails: "", cleanShaven: "",
  hairPins: "", tobacco: "", employeeSign: "", correctiveAction: "",
});

export default function PersonalHygieneEditPage() {
  ...
  useEffect(() => {
    docsApi.get("personalhygienecheckup", recordId)
      .then((res) => {
        const d = res.data;
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await docsApi.update("personalhygienecheckup", recordId, {
        check_date: date,
        area,
        ...
        rows: rows.filter((r) => r.selected && (r.name || CHECK_FIELDS.some(({ field }) => r[field]))).map((r) => ({
          name: r.name,
          respiratory: r.respiratory, skin_disease: r.skinDisease, wounds: r.wounds,
          ear_nose_throat: r.earNoseThroat, gowning: r.gowning, hand_hygiene: r.handHygiene,
          nails: r.nails, clean_shaven: r.cleanShaven, hair_pins: r.hairPins, tobacco: r.tobacco,
          employee_sign: r.employeeSign, corrective_action: r.correctiveAction,
        })),
      });
  ...
  if (loadingData) return (
    <DocFormShell title="Personal Hygiene & Health Checkup" docNo="CFPLA.C7.F.39" subtitle="Frequency: Daily" icon={HeartPulse} width="full">
  ...
  return (
    <DocFormShell title="Personal Hygiene & Health Checkup" docNo="CFPLA.C7.F.39" subtitle="Frequency: Daily" icon={HeartPulse} width="full">
      <DocSection title="Period & Area">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input type="text" value={area} readOnly className="input-base opacity-70 bg-cream-50" />
          </div>
        </div>
      </DocSection>
        ...
        <div className="surface-card p-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {CHECK_FIELDS.map(({ field, label }) => {
              ...
              return fails > 0 ? (
                <span key={field} className="text-[11px] font-semibold bg-danger-50 text-danger-600 px-2 py-0.5 rounded-full">✕ {label}: {fails}</span>
              ) : null;
            })}
          </div>
        </div>
      ...
      <DocSection title="Hygiene Roster" description={`${rows.length} employees`} bleed
          ...
          <table className="w-full text-xs">
            <thead className="bg-cream-100/70">
              <tr className="border-b border-cream-300">
                ...
                {GROUPS.map((g) => (
                  <th key={g.name} colSpan={g.span} className={`px-2 py-2 text-center text-[11px] font-bold ${groupHeaderClass(g.tone)}`}>{g.name}</th>
                ))}
                ...
                <th rowSpan={2} className="px-2 py-2 text-center w-14"></th>
              </tr>
              <tr className="border-b border-cream-300">
                {CHECK_FIELDS.map(({ field, label, tone }) => (
                  <th key={field} className={`px-1 py-1.5 text-center w-20 text-[10px] font-semibold leading-tight ${groupHeaderClass(tone)}`}>{label}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-cream-300">
                ...
                return (
                  <tr key={row.id} className={!row.selected ? "opacity-40 bg-cream-200/60" : hasIssue ? "bg-danger-50/30" : "hover:bg-cream-100/60"}>
                    <td className="px-2 py-1 text-center">
                      <input type="checkbox" checked={row.selected} onChange={() => toggleRowSelected(row.id)} className="h-4 w-4 accent-brand-500 cursor-pointer" />
                    </td>
                    ...
                    <td className="px-1 py-1">
                      <input type="text" value={row.correctiveAction} onChange={(e) => updateRow(row.id, "correctiveAction", e.target.value)}
                        disabled={!hasIssue} placeholder={hasIssue ? "Describe action…" : "—"}
                        className="input-base !py-1 !px-2 text-xs disabled:bg-cream-200/60 disabled:text-ink-300" />
                    </td>
                    <td className="px-1 py-1 text-center">
                      <div className="flex flex-col items-center gap-1">
                        <button onClick={() => markRowAllOK(row.id)} className="text-[9px] font-semibold bg-success-50 text-success-700 px-1.5 py-0.5 rounded hover:bg-success-100 whitespace-nowrap">All ✓</button>
                        <button onClick={() => removeRow(row.id)} className="inline-flex items-center justify-center w-6 h-6 rounded-md text-ink-400 hover:text-danger-600 hover:bg-danger-50"><X className="w-3 h-3" /></button>
                      </div>
                    </td>
                  </tr>
                );
              ...
              <tr className="bg-cream-100/60">
                ...
                <td colSpan={10} className="px-1 py-1">
                  <input type="text" value={observation} onChange={(e) => setObservation(e.target.value)} className="input-base !py-1 !px-2 text-xs" placeholder="Overall observation notes…" />
                </td>
                <td colSpan={3}></td>
              </tr>
            </tbody>
      ...
      <div className="surface-card p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <button type="button" onClick={handleSubmit} disabled={submitting} className="btn-primary inline-flex items-center gap-2">
